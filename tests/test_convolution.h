...
void test_convolution() {
    ...
    ffml_tensor * inputs = ffml_tensor_create(3, {1,5,5,0}, "inputs");

    // auto convmlp = new ConvMLP(pool, 1, 5, 5, {
    //     {2, 3, 2}, // # of filters, kernel side, pool side
    // });

    auto convlayer = new ConvLayer(pool, 1, 5, 5, 2, 3, "convlayer0");

    auto out = convlayer->call(inputs);
    ...
    for(uint64_t i = 0; i < 3; i++) {
        for(uint64_t j = 0; j < 3; j++) {
            for(uint64_t in_chan = 0; in_chan < 1; in_chan++) {
                for (uint64_t out_chan = 0; out_chan < 2; out_chan++) {
    ...
    // for(uint64_t i = 0; i < 3; i++) {
    //     for(uint64_t j = 0; j < 3; j++) {
    //         for(uint64_t in_chan = 0; in_chan < 2; in_chan++) {
    //             for (uint64_t out_chan = 0; out_chan < 2; out_chan++) {
    ...
    // test_tensor_data_flat_equal(convlayer0_conv, {81.0f, 102.0f, 123.0f, 144.0f, 165.0f, 186.0f, 144.0f, 174.0f, 204.0f, 234.0f, 264.0f, 294.0f, 225.0f  }); // ...

    // auto convlayer0_pool = ffml_get_tensor_by_name(cgraph, "maxpool0_pooled");
    // test_tensor_data_flat_equal(convlayer0_pool, {174.0f, 234.0f, 294.0f, 372.0f, 468.0f, 564.0f, 642.0f, 774.0f, 906.0f, 188.0f, 253.0f, 318.0f, 404.0f, 509.0f, 614.0f  }); //...

    // ffml_debug_print_cgraph_data(cgraph);

    test_tensor_data_flat_almost_equal(convlayer0_filter, {0.0f     });
    test_tensor_grad_flat_almost_equal(convlayer0_filter, {60.0f, 78.0f     });
